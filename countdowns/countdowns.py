...
class Countdowns(commands.Cog):
    ...
    async def category(self, ctx):
        ...
        if not category:
            ...
            await self.db.find_one_and_update(
                {"_id": "config"}, {"$set": {"category": str(category.id)}}, upsert=True
            )
    ...
    async def create(self, ctx, *, time: UserFriendlyTime):
        ...
        try:
            ...
            channel = await ctx.guild.create_voice_channel(
                name=time.arg, category=category
            )
        except discord.Forbidden:
            await ctx.send(
                'Bot has insufficient permissions to manage voice channels and category. Allow the bot to "Manage Channels"'
            )
            return
        ...
        await ctx.send(
            'Countdown created! View the voice channels in the "Countdowns" category.'
        )
    ...
    async def update(self, ctx, *, name):
        ...
        try:
            await self.update(
                data["name"], datetime.fromisoformat(data["date"]), channel
            )
        except discord.Forbidden:
            await ctx.send(
                'Bot has insufficient permissions to manage voice channels. Allow the bot to "Manage Channels"'
            )
            return
    ...
    async def update(self, name, date, channel):
        ...
        if seconds < 0:
            ...
            await self.db.find_one_and_delete(
                {"name": name, "date": date.isoformat(), "channel_id": str(channel.id)}
            )
            return False
        ...
        if months > 1:
            ...
            hours += months * 30 + days * 24 - 1 + minutes / 60 + seconds / 60 / 60
            hours = round(hours)
